...
fun checksumMinMax(input: String) = checkSum(input) { it.max()!! - it.min()!! }

fun checksumDivision(input: String) = checkSum(input) { row ->
    row.map { x ->
        row.map { y -> x to y }
...
inline fun checkSum(input: String, rowEvaluator: (List<Int>) -> Int) = input
        .lines()
...
fun main(args: Array<String>) {
    println(checksumMinMax(DAY_2_INPUT.trimMargin()))
    println(checksumDivision(DAY_2_INPUT.trimMargin()))
}